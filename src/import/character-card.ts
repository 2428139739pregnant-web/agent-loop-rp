...
function parseLorebookEntry(value: JsonValue, index: number, version: CharacterCardVersion): ImportedLorebookEntry {
  ...
  // ─── SillyTavern 扩展参数透传(docs/st-adaptation-research.md §1/§8)───
  // ST 导出的角色卡把 selectiveLogic(数字 0-3)/ probability / useProbability /
  // position(数字 0-7)放在 extensions 里。宽松解析:非法值静默回落 ST 默认,
  // 不让旧卡片/三方卡因为这些字段挂掉(与上面 spec 校验的严格字段分开处理)。
  const lenientNumber = (v: unknown): number | undefined =>
    typeof v === 'number' && Number.isFinite(v) ? v : undefined
  const lenientBool = (v: unknown): boolean | undefined => typeof v === 'boolean' ? v : undefined
  const logicRaw = extensions.selectiveLogic ?? entry.selective_logic ?? entry.selectiveLogic
  const logicNum = lenientNumber(logicRaw)
  const secondaryLogic = logicNum === 1 || logicRaw === 'NOT_ALL' ? 'not-all'
    : logicNum === 2 || logicRaw === 'NOT_ANY' ? 'not-any'
    : logicNum === 3 || logicRaw === 'AND_ALL' ? 'and-all'
    : 'and-any'
