...
The main send path also awaits `WORLD_INFO_ACTIVATED` after the resolved World Info set is known and before the response agent snapshots its prompt. The event uses stable merged-book paths as `uid`; any Tavern Helper injection or variable mutation completed by that hook is re-read during the same response generation, without an additional model call.
