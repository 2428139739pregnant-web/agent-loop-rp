...
The official `image` generation input is not consumed or forwarded by this standalone bridge. Active external preset selection, `execute`, `getpreset`, `getqr`, provider proxy-preset lookup, image input handling, and dynamic keyed World Info activation initiated from inside the iframe host surface remain unsupported. The private generation path does, however, run the current normalized Session World Info through the local deterministic ST matcher before the provider call: constant entries and currently matching card, external, and Tavern Helper-source entries are collected without the semantic worldbook agent or another LLM call, then assembled into the supported prompt buckets. This is a deterministic pre-generation compatibility lane, not a complete SillyTavern prompt-manager or dynamic World Info runtime.
