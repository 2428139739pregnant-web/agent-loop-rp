...
The host persists these namespaces in the Session and returns them to the iframe on load. `message_id` is currently resolved against a flat message projection, not a SillyTavern message tree. The bridge includes the Tavern Helper event surface (`eventOn`, `eventOnce`, `eventMakeFirst`, `eventMakeLast`, removal/clear helpers, and awaitable `eventEmit`) plus the current `tavern_events` names. Host lifecycle delivery uses an acknowledgement handshake: generation waits for asynchronous listeners and their pending prompt/variable RPC mutations before entering the response request. Exact ST payloads for every event, arbitrary host extension callbacks, and real ST message-tree semantics are still not implemented. The `SillyTavern` object contains compatibility placeholders for APIs that are outside this safe iframe slice.
