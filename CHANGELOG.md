...
- 补齐 `WORLDINFO_FORCE_ACTIVATE`：卡片或酒馆助手通过事件传入原生 `{ world, uid }` 条目时，会解析为当前合并世界书的 canonical path，并加入当前生成轮的强制激活集合；不增加 LLM 调用。
...
- 对齐 World Info 事件身份：角色卡内嵌书、独立世界书和酒馆助手书保留展示用 `world` 名称，`WORLD_INFO_ACTIVATED`/`WORLDINFO_ENTRIES_LOADED` 优先输出来源 `uid`，同时继续保留 canonical path 作为宿主内部身份。
...
- 修正 `WORLD_INFO_ACTIVATED` 的激活集合：现在同时发送本轮保留的蓝灯常驻条目、关键词命中条目和 `activewi` 强制条目，并补齐 key、次关键词、选择逻辑、position、概率、递归和包含组元数据；不增加 LLM 调用。
- 将 World Info 事件投影回归测试纳入 `pnpm test:agents`，避免后续兼容改动只通过局部测试而漏出。
...
- 接通主发送链的官方 `GENERATE_BEFORE_COMBINE_PROMPTS` 生成前钩子：在 `GENERATION_AFTER_COMMANDS` 后等待卡片/酒馆助手脚本完成提示词注入，再评估函数式注入过滤器，确保本轮新建的注入不会错过当前请求；全程不增加 LLM 调用。
